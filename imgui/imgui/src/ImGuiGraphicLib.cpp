...
ImGuiGraphicLib &	ImGuiGraphicLib::operator=(ImGuiGraphicLib const & imgl) {
	this->_window = imgl.getWin();
	this->_windowSize = imgl.getSize();
	this->_gl_context = imgl.getContext();
	this->_shader = imgl.getShader();
	this->_camera = imgl.getCamera();
	this->_isOpen = imgl.IsOpen();
	this->_errorCode = imgl.PollError();
	this->_errorStr = imgl.GetError();
	this->_proj = imgl.getProj();
	this->_view = imgl.getView();
	this->VAO = imgl.getVAO();
	this->VBO = imgl.getVBO();
	this->EBO = imgl.getEBO();
	this->_vertices = imgl.getVertices();
	this->_indices = imgl.getIndices();
	return *this;
}
...
void ImGuiGraphicLib::DrawSquare(Vec2<float> position, Vec2<float> size, Color color, u32 score) {
	(void) score;
	u32 currentIndex = 0;
	if (_indices.size() > 0)
...
int ImGuiGraphicLib::PollError() const {
	return _errorCode;
}

std::string ImGuiGraphicLib::GetError() const {
	return _errorStr;
}

bool ImGuiGraphicLib::IsOpen() const {
	return _isOpen;
}

SDL_Window * ImGuiGraphicLib::getWin() const {
	return _window;
}

Vec2<u32>	ImGuiGraphicLib::getSize() const {
	return _windowSize;
}

SDL_GLContext ImGuiGraphicLib::getContext() const {
	return _gl_context;
}

Shader *	ImGuiGraphicLib::getShader() const {
	return _shader;
}

Camera *	ImGuiGraphicLib::getCamera() const {
	return _camera;
}

glm::mat4	ImGuiGraphicLib::getProj() const {
	return _proj;
}

glm::mat4	ImGuiGraphicLib::getView() const {
	return _view;
}

u32			ImGuiGraphicLib::getVAO() const {
	return VAO;
}

u32			ImGuiGraphicLib::getVBO() const {
	return VBO;
}

u32			ImGuiGraphicLib::getEBO() const {
	return EBO;
}

std::vector<Vertex> ImGuiGraphicLib::getVertices() const {
	return _vertices;
}

std::vector<u32> ImGuiGraphicLib::getIndices() const {
	return _indices;
}
