...
SDLGraphicLib &	SDLGraphicLib::operator=(SDLGraphicLib const & sgl) {
	this->_window = sgl.getWin();
	this->_renderer = sgl.getRenderer();
	this->_isOpen = sgl.IsOpen();
	this->_errorCode = sgl.PollError();
	this->_errorStr = sgl.GetError();
	return *this;
}
...
void SDLGraphicLib::DrawSquare(Vec2<float> position, Vec2<float> size, Color color, u32 score) {
...
int SDLGraphicLib::PollError() const {
	return _errorCode;
}

std::string SDLGraphicLib::GetError() const {
	return _errorStr;
}

bool SDLGraphicLib::IsOpen() const {
	return _isOpen;
}

SDL_Window * SDLGraphicLib::getWin() const {
	return _window;
}

SDL_Renderer * SDLGraphicLib::getRenderer() const {
	return _renderer;
}
